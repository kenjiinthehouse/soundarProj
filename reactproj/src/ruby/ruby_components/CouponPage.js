...
function CouponPage(props) {
  ...
  function insertNewCoupon(item) {
    ...
    axios
      .post('http://localhost:5566/coupon/client/insert', data)
      .then(() => {
        getClientCoupon();
      })
      .catch((error) => {
  ...
  function initData() {
    ...
    const newData = couponPoolData.filter(
      (item) => clientCouponSid.indexOf(item.sid) === -1
    );
    console.log(newData);
  ...
  useEffect(() => {
    let unUsedData = clientCouponDisplay.filter((item) => {
      return item.used === 0;
    });
    let usedData = clientCouponDisplay.filter((item) => {
      return item.used === 1;
    });
    let clientCouponSid = clientCouponDisplay.map((item) => {
      return item.coupon_sid;
    });
    const couponData = couponPoolData.filter(
      (item) => clientCouponSid.indexOf(item.sid) === -1
    );
        ...
        <div className="ru-coupon-page-wrap">
          ...
          <main className="ru-coupon-main w-100">
            <div className="ru-coupon-switch-tab d-flex justify-content-around">
              <div
                className={
                  couponState === 0 ? 'coupon-tab coupon-active' : 'coupon-tab'
                }
                onClick={() => {
                  setCouponState(0);
                }}
              ...
              <div
                className={
                  couponState === 1 ? 'coupon-tab coupon-active' : 'coupon-tab'
                }
                onClick={() => {
                  setCouponState(1);
                }}
              ...
              <div
                className={
                  couponState === 2 ? 'coupon-tab coupon-active' : 'coupon-tab'
                }
                onClick={() => {
                  setCouponState(2);
                }}
            ...
            <div
              className={
                couponState === 0
                  ? 'ru-coupon-container d-flex flex-wrap justify-content-start'
                  : 'ru-coupon-container px-2'
              }
            >
              {couponState === 0
                ? CouponDisplay.map((item) => {
                    return (
                      <div className="ru-collect-coupon d-flex flex-column">
                        ...
                        <div
                          className="collect-coupon-btn p-2"
                          onClick={() => {
                      ...
                      <div className="ru-my-coupon d-flex justify-content-between">
                        ...
                        <div className="coupon-use-detail d-flex flex-column justify-content-around">
                          <div className="coupon-exp-date">
                            ...
                            <div>
                              <span className="pr-2">
                                {setTimeFormat(item.start_date)}
                              </span>
                              ~
                              <span className="pl-2">
                                {item.end_date
                                  ? setTimeFormat(item.end_date)
                                  : '無期限'}
                              </span>
                            </div>
                          </div>
