...
function OrderPage(props) {
  ...
  function submitComments() {
    ...
    axios
      .post('http://localhost:5566/comment/insert', singleComment)
      .then((res) => {
  ...
  function getOrderData() {
    axios
      .get('http://localhost:5566/order/get', {
        ...
        <div className="row ru-odPage-display">
          ...
          <div className="ru-odPage-option-btn d-flex justify-content-around w-100">
            <div
              className={
                orderStatus === 0
                  ? 'ru-odPage-order-status order-status-active'
                  : 'ru-odPage-order-status'
              }
          ...
          {orderDisplay.filter((item) => item.status === orderStatus).length ===
          0 ? (
            <div className="text-center mt-5">查無資料</div>
          ) : null}
          {orderDisplay
            .filter((item) => item.status === orderStatus)
            .map((value) => {
              return (
                      ...
                      <div className="ru-odPage-order-pic d-flex justify-content-around">
                        ...
                        <div className="ru-odPage-pic-display">
                          <img
                            alt="pic"
                            src={`${
                              value.pic_url[0].indexOf('http') === -1
                                ? `/reckie_img/${value.pic_url[0]}`
                                : value.pic_url[0]
                            }`}
                          />
                        </div>
                        {value.pic_url.length === 1 ? null : (
                          <div className="ru-odPage-pic-display">
                            <img
                              alt="pic"
                              src={`${
                                value.pic_url[1].indexOf('http') === -1
                                  ? `/reckie_img/${value.pic_url[1]}`
                                  : value.pic_url[1]
                              }`}
                            />
                            {value.pic_url.length - 2 !== 0 ? (
                              <div className="ru-odPage-pic-mask">
                                <div className="ru-odPage-pic-num">
                                  +{value.pic_url.length - 2}
                                </div>
                              </div>
                            ) : (
                              ''
                            ...
                            <div className="d-flex justify-content-between align-items-center">
                              ...
                              <div className="ru-odPage-progress-arrive-state d-flex flex-column align-items-center w-25">
                                ...
                                <div className="ru-odPage-progress-text">
                                  ...
                                  <div>
                                    {value.arrive_time
                                      ? setTimeFormat(value.arrive_time)
                                      : null}
                                  </div>
                                </div>
                              </div>
                            ...
                            {value.products.map((item) => {
                              return (
                                <div className="ru-odPage-pd-card d-flex justify-content-between">
                                  <div
                                    className="ru-odPage-pd-img"
                                    style={{
                                      backgroundImage: `url(${
                                        item.pic_url.indexOf('http') === -1
                                          ? `/reckie_img/${item.pic_url}`
                                          : item.pic_url
                                      })`,
                                    }}
                                  ></div>
                              ...
                              {value.comment.map((item) => {
                                return (
                                  <div className="ru-odPage-pd-comment d-flex justify-content-around">
                                    ...
                                    {commentData.filter(
                                      (el) => el.comment_id === item.comment_id
                                    )[0].isCommented ? null : (
                                      <div
                                        className="btn ru-comment-btn mt-auto"
                                        onClick={() => {
                                          let newData = commentData.map(
                                            (el) => {
                                              if (
                                                el.comment_id ===
                                                item.comment_id
